 ...
 * 通过后端代理调用豆包 AI
 */
function callDoubaoAPI(wardrobe, occasion, weather, temperature, userStyle, lockedItemIds) {
 ...
 */
function generateOutfitSuggestion(wardrobe, occasion, weather, temperature, userStyle, lockedItemIds = []) {
  const lockedItems = wardrobe.filter(item => lockedItemIds.includes(item.id));

  // 尝试调用 AI 后端
  return callDoubaoAPI(wardrobe, occasion, weather, temperature, userStyle, lockedItemIds)
    .then(result => {
